...
import { savePdfToGoogleDrive } from "../utils/googleDrive";
...
const isSensitive = (id) =>
  SENSITIVE.some((s) => id.toLowerCase().includes(s));

function prepareW4Answers(answers) {
  const children = Number(answers.children_under_17 || 0);
  const otherDependents = Number(answers.other_dependents_count || 0);

  const qualifyingChildrenAmount = children * 2200;
  const otherDependentsAmount = otherDependents * 500;
  const dependentsAmount = qualifyingChildrenAmount + otherDependentsAmount;

  let multipleJobsMethod =
    "Skip — I have only one job and am Single or MFJ with non-working spouse";

  if (answers.has_multiple_jobs === "Yes" || answers.spouse_works === "Yes") {
    if (answers.exactly_two_similar_jobs === "Yes") {
      multipleJobsMethod =
        "Check this box (ONLY if there are exactly 2 jobs total with similar pay)";
    } else {
      multipleJobsMethod =
        "Use IRS Tax Withholding Estimator at IRS.gov/W4App (most accurate)";
    }
  }
  ...
  return {
    ...
    qualifying_children_amount: qualifyingChildrenAmount
      ? String(qualifyingChildrenAmount)
      : "",

    other_dependents_amount: otherDependentsAmount
      ? String(otherDependentsAmount)
      : "",

    dependents_amount: dependentsAmount ? String(dependentsAmount) : "",

    multiple_jobs_method: multipleJobsMethod,
  };
}
...
export default function FormFillPage() {
  ...
  const [driveSavedLink, setDriveSavedLink] = useState("");  
    ...
    fetch(`${API}/api/forms/${formId}`)
      ...
      .then((d) => {
        if (d.questions?.length) {
          ...
          setQuestions(buildGeneric(form?.fields || []));
        }
      })
      .catch(() => setQuestions(buildGeneric(form?.fields || [])));
  }, [formId, form]);

  // Auto-fill from situation text passed via navigate state
  useEffect(() => {
    const situation =
      location.state?.prefillSituation || location.state?.situation;
  ...
  }, [questions, location.state, formId]);
  ...
  const visibleQuestions = questions
    ? questions.filter((q) => {
        if (q.show_if) {
          ...
          if (depVal !== q.show_if.value) return false;
        }

        if (q.show_if_any) {
          const shouldShow = q.show_if_any.some((rule) => {
            const depVal = answers[rule.field] || "";
  ...
  const filled = visibleQuestions
    ? visibleQuestions.filter((q) => {
        if (q.is_note) return false;
        const v = answers[q.id];
        return v && String(v).trim();
      }).length
    : 0;

  const total = visibleQuestions
    ? visibleQuestions.filter((q) => !q.is_note).length
    : 0;
  ...
  async function handleExtract() {
    ...
    setEmailSent(false);
    setLoading(true);
    setLoadingMsg("Reading your document with AI…");
  ...
  async function handleGenerate() {
    ...
    try {
      const preparedAnswers =
        formId === "w4" ? prepareW4Answers(answers) : answers;

      if (saveData) {
        const safe = Object.fromEntries(
          Object.entries(preparedAnswers).filter(([k]) => !isSensitive(k))
        );
  ...
  try {
    ...
    if (!accessToken) {
      throw new Error("Please sign in with Google again to allow Drive access.");
    }

    const fileName = `${form.short_name || formId}_DocuLyft.pdf`;

    const savedFile = await savePdfToGoogleDrive(
      pdfUrl,
      fileName,
      accessToken
    );

    setDriveSavedLink(savedFile.webViewLink || "");
  } catch (e) {
    setError(e.message || "Failed to save to Google Drive.");
  } finally {
  ...
  async function handleEmail() {
    ...
    setEmailSent(false);
    setLoading(true);
    setLoadingMsg("Sending email…");

    try {
      const absoluteDownloadUrl = pdfUrl.startsWith("http")
        ? pdfUrl
        : `${API}${pdfUrl}`;

      const res = await fetch(`${API}/api/email-pdf`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
            ...
            form_name: form?.form_name || "Your DocuLyft PDF",
        }),
      });
        ...
        <div className="ff-split-screen">
          <div className="ff-split-left">
            {!entryMode && (
              <div style={{ padding: "28px 24px" }}>
                <div
                  ...
                  style={{ color: category?.color, marginBottom: 6 }}
                >
                  {category?.label}
                </div>
                ...
                >
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p>
                        Drop any file — prior form, passport, ID, notes. AI
                        reads it and pre-fills all the fields for you.
                      </p>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p>
                        Enter your details directly. The official form is shown
                        on the right for reference.
                      </p>
                      <div className="ff-mode-badge ff-mode-badge-manual">
                        Manual entry
                      </div>
                    </div>
                  </button>
            ...
            {entryMode === "upload" && (
              ...
              >
                ...
                <p className="ff-subtitle" style={{ margin: 0 }}>
                  AI will extract your information and pre-fill the form. Review
                  everything after.
                </p>
                ...
                <div
                  className={`ff-drop-zone${dragOver ? " drag-over" : ""}${
                ...
                >
                  ...
                  {uploadedFile ? (
                    <>
                      ...
                      <div className="ff-drop-filename">
                        {uploadedFile.name}
                      </div>
                      <div className="ff-drop-filesize">
                    ...
                    <>
                      ...
                      <div className="ff-drop-label">
                        Drop file here or <span>click to browse</span>
                      </div>

                      <div className="ff-drop-types">
                        PDF, DOC, TXT, PNG, JPG
                      </div>
                    </>
                  )}
            ...
            {entryMode === "manual" && (
              <>
                ...
                <div className="ff-split-form-title">
                  <div
                    className="fa-eyebrow"
                    style={{ color: category?.color, marginBottom: 4 }}
                  >
                    {category?.label}
                  </div>
                ...
                {error && (
                  <div className="fa-error" style={{ margin: "0 18px 4px" }}>
                    {error}
                  </div>
                )}
                ...
                {questions && (
                  <div className="ff-fields-scroll">
                    {visibleQuestions.map((q) => {
                      ...
                      const sens = q.sensitive || isSensitive(q.id);
                      ...
                      return (
                        ...
                        >
                          <label
                            className="ff-field-label-inline"
                            htmlFor={`ff_${q.id}`}
                          >
                            {q.label}
                            {q.required && (
                ...
                <div className="ff-split-bottom">
                  <div className="ff-save-row">
                    ...
                    <div>
                      ...
                      <span className="ff-save-sub">
                        {" "}
                        — SSNs & sensitive fields never saved
                      </span>
                    </div>
                  </div>

                  {error && (
                    <div className="fa-error" style={{ marginTop: 8 }}>
                      {error}
                    </div>
                  )}
          ...
          <div className="ff-split-right">
            <div className="ff-pdf-viewer-header">
              <span className="ff-pdf-viewer-title">
            ...
            {form.pdf_path ? (
              ...
              >
                <div className="ff-pdf-no-form">
                  ...
                  >
                    Fill in your details on the left, then click Generate to
      ...
      {step === "done" && (
        <div className="ff-body">
          ...
          <div className="ff-deliver-grid" style={{ marginTop: 28 }}>
            <a
              href={pdfUrl}
              download={`${form.short_name}_DocuLyft.pdf`}
              className="ff-deliver-card"
            ...
            <div className="ff-deliver-card">
              ...
              {error && (
                <div className="fa-error" style={{ marginTop: 10 }}>
                  {error}
                </div>
              )}
            </div>
          ...
          <div className="ff-deliver-also" style={{ marginTop: 20 }}>
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="ff-preview-again"
            >
              View in browser ↗
            </a>

            <span style={{ color: "var(--ink-muted)", margin: "0 12px" }}>
              ·
            </span>
            ...
            <button
              className="ff-preview-again"
              onClick={() => navigate(`/category/${form.category}`)}
            >
              Fill another form
            </button>
          </div>
          ...
          <div className="ff-reminder" style={{ marginTop: 24 }}>
            ...
            <div>
              <strong>Before submitting:</strong> Add any sensitive fields,
              signatures, or payment info by hand. Submit only through{" "}
              <strong>{form.agency}'s official channel</strong>.
            </div>
          </div>
        </div>
      )}
