...
FORM_PDFS = {
    "w9":    FORMS_DIR/"tax/w9.pdf", "w4": FORMS_DIR/"tax/w4.pdf",
    "w2":    FORMS_DIR/"tax/w2.pdf", "1040": FORMS_DIR/"tax/1040.pdf",
    "1040x": FORMS_DIR/"tax/1040x.pdf", "1040es": FORMS_DIR/"tax/1040es.pdf",
    "1040s1":FORMS_DIR/"tax/1040s1.pdf","1040s2":FORMS_DIR/"tax/1040s2.pdf",
    "1040sr":FORMS_DIR/"tax/1040sr.pdf","1065": FORMS_DIR/"tax/1065.pdf",
    "1095a": FORMS_DIR/"tax/1095a.pdf","1095b": FORMS_DIR/"tax/1095b.pdf",
    "1095c": FORMS_DIR/"tax/1095c.pdf","1096":  FORMS_DIR/"tax/1096.pdf",
    "1099div":FORMS_DIR/"tax/1099div.pdf","1099g":FORMS_DIR/"tax/1099g.pdf",
    "1099int":FORMS_DIR/"tax/1099int.pdf","1099misc":FORMS_DIR/"tax/1099misc.pdf",
    "1099nec":FORMS_DIR/"tax/1099nec.pdf","1099r":FORMS_DIR/"tax/1099r.pdf",
    "1120":  FORMS_DIR/"tax/1120.pdf","1120s": FORMS_DIR/"tax/1120s.pdf",
    "2210":  FORMS_DIR/"tax/2210.pdf","2290":  FORMS_DIR/"tax/2290.pdf",
    "2441":  FORMS_DIR/"tax/2441.pdf","3800":  FORMS_DIR/"tax/3800.pdf",
    "4506":  FORMS_DIR/"tax/4506.pdf","4506t": FORMS_DIR/"tax/4506t.pdf",
    "4868":  FORMS_DIR/"tax/4868.pdf","5329":  FORMS_DIR/"tax/5329.pdf",
    "5695":  FORMS_DIR/"tax/5695.pdf","6251":  FORMS_DIR/"tax/6251.pdf",
    "706":   FORMS_DIR/"tax/706.pdf", "709":   FORMS_DIR/"tax/709.pdf",
    "720":   FORMS_DIR/"tax/720.pdf", "8606":  FORMS_DIR/"tax/8606.pdf",
    "8822":  FORMS_DIR/"tax/8822.pdf","8822b": FORMS_DIR/"tax/8822b.pdf",
    "8829":  FORMS_DIR/"tax/8829.pdf","8889":  FORMS_DIR/"tax/8889.pdf",
    "8949":  FORMS_DIR/"tax/8949.pdf","8960":  FORMS_DIR/"tax/8960.pdf",
    "8962":  FORMS_DIR/"tax/8962.pdf","940":   FORMS_DIR/"tax/940.pdf",
    "941":   FORMS_DIR/"tax/941.pdf", "944":   FORMS_DIR/"tax/944.pdf",
    "schedA":FORMS_DIR/"tax/schedA.pdf","schedB":FORMS_DIR/"tax/schedB.pdf",
    "schedC":FORMS_DIR/"tax/schedC.pdf","schedD":FORMS_DIR/"tax/schedD.pdf",
    "schedE":FORMS_DIR/"tax/schedE.pdf","schedF":FORMS_DIR/"tax/schedF.pdf",
    "ss4":   FORMS_DIR/"tax/ss4.pdf", "w7":    FORMS_DIR/"tax/w7.pdf",
...
def _fill_fields_exact(pdf_path: Path, field_values: dict, output_path: Path) -> bool:
    """
    Writes field values by exact full field name using pypdf's AcroForm.
    This bypasses update_page_form_field_values which does partial matching.
    """
    try:
        ...
        # Walk ALL annotations on ALL pages and match by /T (field name) or /TM
        filled = 0
        for page in writer.pages:
            ...
            for annot_ref in page["/Annots"]:
                ...
                if annot.get("/Type") == "/Annot" and annot.get("/Subtype") == "/Widget":
                    # Get the full field path by walking up /Parent
                    full_name = _get_full_field_name(annot)
                    if full_name in field_values:
                        val = field_values[full_name]
                        annot.update({
                            NameObject("/V"): create_string_object(val),
                            NameObject("/AS"): create_string_object(val),
                        })
                        filled += 1
...
def generate_filled_pdf(form_id, form_name, agency, answers, questions, output_path):
    ...
    if official and official.exists() and field_map:
        # Build values dict: pdf_field_name -> answer value
        values = {}
    ...
    if official and official.exists():
        try:
            _official_plus_appendix(official, form_name, agency, answers, questions, output_path)
...
def _formatted_data_pdf(form_name, agency, answers, questions, dest):
    ...
    NAVY=colors.HexColor("#0d1f3c"); GRAY=colors.HexColor("#6b7280")
    LGRAY=colors.HexColor("#9ca3af"); AMBER=colors.HexColor("#92400e")
    AMBBG=colors.HexColor("#fffbeb"); AMBBRD=colors.HexColor("#fcd34d")

    title_s=ParagraphStyle("T",fontSize=22,fontName="Helvetica-Bold",textColor=NAVY,spaceAfter=2)
    sub_s  =ParagraphStyle("S",fontSize=10,fontName="Helvetica",textColor=GRAY,spaceAfter=4)
    h2_s   =ParagraphStyle("H2",fontSize=14,fontName="Helvetica-Bold",textColor=NAVY,spaceBefore=20,spaceAfter=8)
    lbl_s  =ParagraphStyle("L",fontSize=8,fontName="Helvetica-Bold",textColor=GRAY,spaceAfter=1,leading=10)
    val_s  =ParagraphStyle("V",fontSize=12,fontName="Helvetica",textColor=NAVY,spaceAfter=12,leading=16)
    emp_s  =ParagraphStyle("E",fontSize=12,fontName="Helvetica",textColor=LGRAY,spaceAfter=12,leading=16)
    note_s =ParagraphStyle("N",fontSize=8,fontName="Helvetica",textColor=GRAY,leading=13)
    warn_s =ParagraphStyle("W",fontSize=9,fontName="Helvetica-Bold",textColor=AMBER)
    ...
    meta=Table([["Form",form_name],["Agency",agency],
        ["Status","Review all fields, add sensitive info by hand, sign and submit officially"]],
        colWidths=[1.1*inch,5.8*inch])
    meta.setStyle(TableStyle([("FONTNAME",(0,0),(0,-1),"Helvetica-Bold"),("FONTSIZE",(0,0),(-1,-1),10),
        ("TEXTCOLOR",(0,0),(0,-1),GRAY),("TEXTCOLOR",(1,0),(1,-1),NAVY),
        ("BOTTOMPADDING",(0,0),(-1,-1),5),("VALIGN",(0,0),(-1,-1),"TOP")]))
    story.append(meta); story.append(Spacer(1,14))

    wt=Table([[Paragraph("⚠ Important",warn_s),
        Paragraph("Review every field. Add sensitive info (SSN, alien numbers, signatures) by hand. Sign where required. Submit only through the official agency.",note_s)]],
        colWidths=[0.9*inch,6.0*inch])
    wt.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),AMBBG),("BOX",(0,0),(-1,-1),1,AMBBRD),
        ("TOPPADDING",(0,0),(-1,-1),10),("BOTTOMPADDING",(0,0),(-1,-1),10),
        ("LEFTPADDING",(0,0),(-1,-1),12),("RIGHTPADDING",(0,0),(-1,-1),12),
        ("VALIGN",(0,0),(-1,-1),"TOP")]))
    story.append(wt); story.append(Spacer(1,18))
    ...
    while i<len(questions):
        ...
        if wide or i+1>=len(questions):
            ...
            t=Table([[Paragraph(q1["label"].upper()+("  *" if q1.get("required") else ""),lbl_s),
                     ...
                     [Paragraph(v1 if v1 else "— not provided —",val_s if v1 else emp_s),
                      Paragraph(v2 if v2 else "— not provided —",val_s if v2 else emp_s)]],
    ...
    for idx,step in enumerate([
        ...
        f"Sign and date where required.",
